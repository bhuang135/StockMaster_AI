...
# =========================================================
def _extract_grounding_sources_markdown(response) -> str:
    """
    Pull real citation URLs out of a Gemini response's grounding_metadata
    (populated when the `google_search` tool actually ran) and format them
    as a separate markdown section. Returns "" if the response wasn't
    actually grounded — we never want to imply live-search backing that
    isn't there.
    """
    try:
        ...
        return "\n\n### Live Search Sources (Gemini + Google Search)\n" + "\n".join(lines)
    except Exception:
        return ""
...
def generate_chat_answer_with_citations(
    ...
    (RAG over this app's own data for the current ticker). When
    `use_search_grounding` is True, the model is ALSO given the
    `google_search` tool so it can fill gaps the retrieved context doesn't
    cover (e.g. a fact that came back "N/A" locally, or something more
    recent than what was retrieved) — but the prompt keeps it scoped to
    only the current ticker/company, so this doesn't turn the chatbot into
    an open-ended general search assistant.
    """
    ...
    if use_search_grounding:
        search_rules = f"""
9. You also have a live Google Search tool available. Use it ONLY to fill
   gaps that the retrieved context above does not cover (for example a
   fact that came back "N/A"/missing locally, or something more recent
   than what was retrieved).
10. Only use Google Search for information about {company_name} ({ticker})
    specifically. Never use it to answer about a different company, a
    different ticker, or anything outside the scope of this question.
11. Clearly mark any claim that came from live search rather than the
    retrieved context above, e.g. "(live search)".
"""
...
2. Every material claim must have inline citations like [1], [2] when it comes from the retrieved context below.
3. If neither the retrieved context nor live search (when available) has enough evidence, say exactly:
...
5. If the question asks for exact live market data beyond the retrieved context and live search isn't available or doesn't cover it, clearly say that live confirmation may require external real-time APIs.
...
"""
    ...
    try:
        ...
        # Grounding tool name/support can differ across SDK/model versions.
        # Fall back to a context-only (ungrounded-tool) answer rather than
    ...
    live_sources_md = _extract_grounding_sources_markdown(response) if use_search_grounding else ""
    if live_sources_md:
        text += live_sources_md
